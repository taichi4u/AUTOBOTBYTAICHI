...
module.exports.run = async function ({ api, event, args }) {
  ...
  const uptimeString = `${years > 0 ? `${years} years ` : ''}${months > 0 ? `${months} months ` : ''}${days % 365 > 0 ? `${days % 365} days ` : ''}${hours > 0 ? `${hours} hours ` : ''}${minutes > 0 ? `${minutes} minutes ` : ''}${seconds} seconds`;

  const prefix = "/";
  const CREATORLINK = "https://www.facebook.com/jenard.viola.1";
  const BOTCREATOR = "Jenard Viola";
  const BOTNAME = "Taichi Bot";
  const FILESOWNER = "Jenard";
